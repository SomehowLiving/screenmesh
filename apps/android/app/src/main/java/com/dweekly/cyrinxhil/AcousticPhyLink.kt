...
class AcousticPhyLink(private val config: SessionConfig) {
    ...
    private fun decodeAvailableLocked(): List<AcousticDecodedFrame> {
        ...
        while (true) {
            ...
            // NOTE (ScreenMesh vendoring change — see NOTICE.md): moved below
            // the guard above so the compiler can smart-cast `bodyPayload` to
            // non-null at the point of assignment. `var frame = bodyPayload`
            // declared *before* the guard (upstream's original ordering)
            // fails to compile under this project's Kotlin version — a `var`
            // doesn't inherit a smart-cast performed on the variable it was
            // assigned from. No behavioral change: same value, same
            // control-flow path, just reordered.
            var frame: ByteArray = bodyPayload
