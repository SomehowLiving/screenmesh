...
function attachHandshake(session: ActiveLanSession, socket: TLSSocket): void {
  ...
  const onData = (chunk: Buffer) => {
    ...
    try {
      const request = JSON.parse(buffered.slice(0, lineEnd)) as { type?: unknown; sessionToken?: unknown };
      if (session.consumed || request.type !== "screenmesh.lan.hello" || !tokenMatches(session.tokenHash, request.sessionToken)) {
        return reject();
      }
