...
- [~] **Android app** (`apps/android`): a hand-ported Kotlin core of `packages/protocol`/`crypto`/`transport` (BouncyCastle for Ed25519/X25519/HKDF, OkHttp for the relay WebSocket + pairing HTTP) plus a `MeshEngine` — including store-carry-forward, `continueOnDevice`, last-write-wins object updates, and secure file drop chunking, not just the original CREATE_OBJECT/SEND_TO_DEVICE minimum — identity/session persistence, and a reference UI, so the phone joins the same workspaces and speaks the same envelope/ratchet protocol as the web PWA and desktop agent — see docs/Android.md for exactly what's ported vs. deferred (Yjs collaborative editing isn't ported — no viable in-scope JVM port exists; object/delivery/ratchet-session state is in-memory only, matching the desktop agent's own scope). **Builds, lints, and — the test that matters — passes a real cross-language interop run against the live TypeScript engine over a real relay, in both directions.** A JDK/Android SDK/Gradle toolchain was set up, a committed Gradle wrapper (`./gradlew assembleDebug`) produces a real debug APK, and `packages/sync/scripts/interop-with-android.ts` + `apps/android/.../InteropSmoke.kt` prove the Kotlin engine actually speaks the same protocol as everything else — envelope sealing/verification, the Double Ratchet, MeshEngine's op encoding, all confirmed wire-compatible, not just reviewed-to-match. Still never run on an actual device/emulator (none exists in this environment) — the Activity/UI/BLE-radio layers remain unexercised. See docs/Android.md's "Verification status" for the precise line.
- [~] Bluetooth LE / Wi-Fi Direct / NFC pairing and transport — real implementations exist in `apps/android`'s `transport/nearby/` (GATT peripheral+central for BLE, `WifiP2pManager` + TCP socket for Wi-Fi Direct, NDEF read/write for NFC), all compile cleanly, and all three are now wired into `MainActivity`: BLE and NFC as nearby pairing bootstraps ("Scan nearby" / "Advertise via BLE" / "Write to NFC tag" — all reuse the same pairing-code string a QR carries, no new trust protocol), Wi-Fi Direct as a raw transport with its `BroadcastReceiver` lifecycle wired ("Scan nearby (Wi-Fi Direct)"). A review pass caught and fixed three real bugs here: a BLE double-connect race, an NFC cold-start intent that was silently dropped, and an NFC "write mode" with no expiry (risking an accidental tag overwrite) — see docs/Android.md. Untested on real hardware, like the rest of `apps/android`
