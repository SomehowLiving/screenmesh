...
- [~] **Android app scaffolded** (`apps/android`): a hand-ported Kotlin core of `packages/protocol`/`crypto`/`transport` (BouncyCastle for Ed25519/X25519/HKDF, OkHttp for the relay WebSocket + pairing HTTP) plus a reduced `MeshEngine`, identity/session persistence, and a bare reference UI, so the phone joins the same workspaces and speaks the same envelope/ratchet protocol as the web PWA and desktop agent — see docs/Android.md for exactly what's ported vs. deferred (Yjs, store-carry-forward, and file chunking aren't ported to MeshEngine yet; object/delivery/ratchet-session state is in-memory only, though identity + session now survive a restart). **Not built or run** — this environment has no JDK/Android SDK/Gradle/device, so nothing beyond careful line-by-line porting against the TS source was possible here.
