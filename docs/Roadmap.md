...
- [~] **Android app** (`apps/android`): a hand-ported Kotlin core of `packages/protocol`/`crypto`/`transport` (BouncyCastle for Ed25519/X25519/HKDF, OkHttp for the relay WebSocket + pairing HTTP) plus a `MeshEngine` — including store-carry-forward, `continueOnDevice`, last-write-wins object updates, and secure file drop chunking, not just the original CREATE_OBJECT/SEND_TO_DEVICE minimum — identity/session persistence, and a reference UI, so the phone joins the same workspaces and speaks the same envelope/ratchet protocol as the web PWA and desktop agent — see docs/Android.md for exactly what's ported vs. deferred (Yjs collaborative editing isn't ported — no viable in-scope JVM port exists; object/delivery/ratchet-session state is in-memory only, matching the desktop agent's own scope). **Now builds and lint-passes for real**: a JDK/Android SDK/Gradle toolchain was set up and a committed Gradle wrapper (`./gradlew assembleDebug`) produces a real debug APK — but the app has never been *run* on a device or emulator (none exists in this environment), so cross-language wire compatibility with the web PWA/desktop agent over a live relay is still unconfirmed. See docs/Android.md's "Verification status" for the precise line between what compiling proves and what it doesn't.
- [~] Bluetooth LE / Wi-Fi Direct / NFC pairing and transport — real implementations exist in `apps/android`'s `transport/nearby/` (GATT peripheral+central for BLE, `WifiP2pManager` + TCP socket for Wi-Fi Direct, NDEF read/write for NFC) and all compile cleanly. A BLE-based nearby pairing bootstrap is wired into `MainActivity` ("Scan nearby" / "Advertise via BLE" — reuses the same pairing-code string a QR carries, no new trust protocol). Wi-Fi Direct and NFC are still not wired into `MainActivity` (no `BroadcastReceiver`/lifecycle hooks registered yet) — see docs/Android.md. Untested on real hardware, like the rest of `apps/android`
